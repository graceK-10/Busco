...
import {
  getPredatorBrandData,
  predatorBrandTabs,
} from "../data/predatorRangeData";
...
const buildPredatorHash = (brandId, busId) =>
  `#/predator?brand=${brandId}${busId ? `&bus=${busId}` : ""}`;
          ...
          <div className="grid gap-0 lg:grid-cols-[1.05fr_0.95fr]">
            <div className="relative min-h-[340px] overflow-hidden lg:min-h-full">
              ...
              <div className="absolute bottom-0 left-0 right-0 p-6 sm:p-8">
                <p className="text-[11px] font-semibold uppercase tracking-[0.26em] text-[#ff8a8a]">
                  {bus.type}
                </p>
                <h3 className="mt-2 text-[clamp(2rem,4vw,3.4rem)] font-black tracking-[-0.05em] text-white">
                  {bus.name}
                </h3>
              </div>
            </div>
            ...
            <div className="relative p-6 sm:p-8 lg:p-10">
              ...
              <div className="pr-14">
                <p className="text-[11px] font-semibold uppercase tracking-[0.28em] text-[#d72626]">
                  Range details
                </p>
                <h4 className="mt-4 text-[1.08rem] leading-8 text-black/75">
                  {bus.blurb}
                </h4>
                ...
                <div className="mt-6 grid gap-4 sm:grid-cols-2">
                  {bus.specs.map((spec) => (
                    <div
                      key={spec.label}
                      className="rounded-[20px] border border-black/8 bg-white px-5 py-5"
                    >
                      <p className="text-[10px] font-semibold uppercase tracking-[0.22em] text-black/40">
                        {spec.label}
                      </p>
                      <p className="mt-2 text-[0.98rem] font-semibold leading-7 text-[#111111]">
                        {spec.value}
                      </p>
                    </div>
                  ))}
                ...
                <div className="mt-7 rounded-[24px] border border-black/8 bg-white px-6 py-6">
                  <h5 className="text-[1rem] font-bold uppercase tracking-[0.18em] text-[#111111]">
                    Bus details
                  </h5>
                ...
                <div className="mt-6 rounded-[24px] border border-black/8 bg-[#111111] px-6 py-6 text-white">
                  ...
                  <ul className="mt-4 space-y-3 text-[0.95rem] leading-7 text-white/72">
                    {bus.optionalFeatures.map((feature, index) => (
                      <li key={`${bus.id}-modal-feature-${index}`}>
                        • {feature}
                      </li>
                    ))}
                  </ul>
...
export default function Predator() {
  ...
  const brandData = useMemo(
    () => getPredatorBrandData(activeBrand),
    [activeBrand],
  );
  const brandBuses = brandData.buses;
  const activeBus =
    brandBuses.find((bus) => bus.id === activeBusId) || brandBuses[0];
      ...
      <main className="relative overflow-hidden bg-[linear-gradient(180deg,#eee8df_0%,#dfd8ce_100%)]">
        ...
        <section className="relative px-6 py-16 sm:px-10 lg:px-16 lg:py-20">
          <div className="mx-auto max-w-[1580px]">
            ...
            >
              ...
              <h1 className="mt-4 text-[clamp(2.8rem,6vw,6rem)] font-black uppercase leading-[0.88] tracking-[-0.06em]">
              ...
              <p className="mx-auto mt-6 max-w-[840px] text-[1.05rem] leading-8 text-black/62">
                Browse the Predator range by chassis brand. The first four buses
                keep the featured layout, while the remaining grid buses open in
                a detailed popup with brochure access and model information.
              </p>
            </motion.div>
            ...
            <div className="mt-10 rounded-[36px] border border-black/8 bg-[#fffdf9] p-6 shadow-[0_30px_90px_rgba(0,0,0,0.10)] backdrop-blur-sm sm:p-8 xl:p-10">
              <div className="mb-8 flex flex-col gap-4 xl:flex-row xl:items-end xl:justify-between">
                ...
                >
                  ...
                  <h2 className="mt-3 text-[clamp(2rem,3vw,3.4rem)] font-black tracking-[-0.04em] text-[#111111]">
                    {brandData.title}
                  </h2>
                  <p className="mt-4 max-w-[760px] text-[1rem] leading-8 text-black/62">
                    {brandData.intro}
                  </p>
                </motion.div>
              ...
              <div className="grid gap-6 xl:grid-cols-[minmax(0,1.4fr)_380px]">
                ...
                >
                  <div className="relative h-[360px] overflow-hidden sm:h-[480px]">
                    ...
                    <div className="absolute bottom-0 left-0 right-0 p-6 sm:p-8">
                      <p className="text-[11px] font-semibold uppercase tracking-[0.28em] text-[#ffb0b0]">
                        {activeBus.type}
                      </p>
                      <h3 className="mt-2 text-[clamp(2rem,4vw,3.4rem)] font-black tracking-[-0.05em] text-white">
                        {activeBus.name}
                  ...
                  <div className="grid gap-6 p-6 sm:p-8 xl:grid-cols-[minmax(0,1fr)_320px]">
                    <div>
                      <p className="text-[1rem] leading-8 text-black/72">
                        {activeBus.blurb}
                      </p>
                      ...
                      <div className="mt-6 grid gap-4 sm:grid-cols-2">
                        {activeBus.specs.map((spec) => (
                          <div
                            key={spec.label}
                            className="rounded-[22px] border border-black/8 bg-[#fbf8f3] px-5 py-5"
                          >
                      ...
                      <div className="mt-8 rounded-[28px] border border-black/8 bg-[#f9f5ef] p-6">
                        <h4 className="text-[1.05rem] font-bold uppercase tracking-[0.18em] text-[#111111]">
                          Bus details
                        </h4>
                        <div className="mt-4 space-y-3 text-[0.98rem] leading-7 text-black/70">
                          {activeBus.details.map((detail, index) => (
                            <p key={`${activeBus.id}-detail-${index}`}>
                              • {detail}
                            </p>
                          ))}
                        </div>
                    ...
                    <div className="flex flex-col gap-5">
                      ...
                      <div className="rounded-[28px] border border-black/8 bg-[#111111] p-6 text-white">
                        ...
                        <h5 className="mt-4 text-[1.2rem] font-semibold leading-7 text-white">
                      ...
                      <div className="rounded-[28px] border border-black/8 bg-[#f3efe8] p-6">
                        ...
                        <ul className="mt-4 space-y-3 text-[0.95rem] leading-7 text-black/68">
                          {activeBus.optionalFeatures.map((feature, index) => (
                            <li key={`${activeBus.id}-feature-${index}`}>
                              • {feature}
                            </li>
                          ))}
                        </ul>
                    ...
                    >
                      <div className="relative h-[240px] overflow-hidden">
                        ...
                        <div className="absolute bottom-0 left-0 right-0 p-5">
                          <p className="text-[10px] font-semibold uppercase tracking-[0.24em] text-[#ffb0b0]">
                            {bus.type}
                          </p>
                          <h4 className="mt-2 text-[1.55rem] font-black tracking-[-0.04em] text-white">
                            {bus.name}
                          </h4>
                          {bus.isPlaceholder && (
                      ...
                      <div className="flex flex-wrap items-center justify-between gap-3 p-5">
                        <p className="max-w-[220px] text-[0.95rem] leading-7 text-black/62">
                          {bus.blurb}
                        </p>
              ...
              <div className="mt-8">
                <div className="mb-5 flex items-center justify-between gap-4">
                  <h4 className="text-[1.1rem] font-bold uppercase tracking-[0.22em] text-[#111111]">
                    More in the {brandData.title} range
                  </h4>
                  <span className="text-[11px] font-semibold uppercase tracking-[0.22em] text-black/45">
                    Grid of remaining buses
                  </span>
                </div>
                    ...
                    >
                      ...
                      >
                        <div className="relative h-[220px] overflow-hidden">
                          ...
                          <div className="absolute bottom-0 left-0 right-0 p-5">
                            <p className="text-[10px] font-semibold uppercase tracking-[0.22em] text-[#ffb0b0]">
                              {bus.type}
                            </p>
                            <h5 className="mt-2 text-[1.45rem] font-black tracking-[-0.04em] text-white">
                              {bus.name}
                            </h5>
                          </div>
                        </div>
                      ...
                      <div className="space-y-4 p-5">
                        ...
                        <p className="text-[0.95rem] leading-7 text-black/65">
                          {bus.blurb}
                        </p>
        ...
        <div className="pointer-events-none absolute bottom-0 right-0 opacity-90">
          <img
            src={redAccent}
            alt=""
            className="w-[220px] sm:w-[260px] lg:w-[420px]"
          />
        </div>
      </main>
